...
// Inlined implementations from value.cpp
namespace vcm {
inline Value::Value ( ) : data(""), update(false)
{
}

inline Value::Value (int i) : update(false)
{
  SetInt(i);
}

inline Value::Value (const std::string &s) : update(false)
{
  SetStr(s);
}

inline Value::Value (const char *s) : update(false)
{
  SetStr(s);
}
...
inline std::string Value::GetStr() const
{
  return this->data;
}
...
inline void Value::SetInt( int i )
{
  char buf[16];
  this->update = true;
...
inline void Value::SetStr( const char *str )
{
  this->update = true;
...
inline void Value::SetStr( const std::string &str )
{
  this->update = true;
  this->data = str;
}

inline Value &Value::operator =( const std::string &str )
{
  this->update = true;
  this->data = str;
  return *this;
