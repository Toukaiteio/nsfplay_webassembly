...
void NSFPlayer::SetPlayFreq (double r)
{
	...
	int quality = (*config)["QUALITY"].GetInt();
	...
	switch (region)
	{
		default:
		case REGION_NTSC:
			clock = (*config)["NTSC_BASECYCLES"].GetInt();
			break;
		case REGION_PAL:
			clock = (*config)["PAL_BASECYCLES"].GetInt();
			break;
		case REGION_DENDY:
			clock = (*config)["DENDY_BASECYCLES"].GetInt();
			break;
	}
	...
	rconv.SetRate(rate);
	rconv.SetFastSkip((*config)["FAST_SEEK"]!=0);
  ...
  void NSFPlayer::Reset ()
  {
    ...
    switch (region)
    {
        default:
        case REGION_NTSC:
            cpu.nes_basecycles = (*config)["NTSC_BASECYCLES"].GetInt();
            break;
        case REGION_PAL:
            cpu.nes_basecycles = (*config)["PAL_BASECYCLES"].GetInt();
            break;
        case REGION_DENDY:
            cpu.nes_basecycles = (*config)["DENDY_BASECYCLES"].GetInt();
            break;
    }
